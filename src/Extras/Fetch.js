...
module.exports = async ( link, keys, m ) => {
  ...
  if ( m ) {
    if ( result.status.toString( ) in Errors ) { 
      await m.reply( new MessageEmbed( ).setTitle( 'Error' ).setDescription( `An Error Occured Contacting \`${ link }\`\n\nCode: (**${ Errors[ result.status.toString( ) ].c }**)\nMeaning: __${ Errors[ result.status.toString( ) ].m }__` ).setColor( 0xff0000 ) );
      return;
    }
    if ( link === 'https://api.mojang.com/users/profiles/minecraft' && result.status.toString( ) === '204' ) { return m.reply( `${ keys.substring( 1 ) } isn't a valid usernanme!` ); }
  }
